...
Let's keep the individual arpeggiated notes, but define a completely new rhythm - note that `struct` is appended to the statement (in general, our rhythm patterns are defined on the left)
```
