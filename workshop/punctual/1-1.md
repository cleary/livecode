...
If you don't specify a position for at least three channels, remaining channels will be created with the last value in the list - this is because `video` is a 3 colour channel output, it will take a value per channel where specified.
...
Notice the line on the right (x-coordinate `0.25`) is a light blue colour, which is the combination of both Green and Blue (which was not specified) channels
