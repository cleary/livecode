...
2. Create a vertically moving `circle` using the `tri` oscillator
...
4. Modify one of our brightness examples, using the `sqr` oscillator to control the circles visibility
...
Then use modify the answer from the previous question to make a circle appear/disappear in time with the beat 
